...
int main(int argc, char *argv[])
{
    ...
    // We are not going to use simulation so just set this to zero
    ...
    agents::DefaultPedOutcomeCalc affected_outcome_calc(&affected_control_ped);
    agents::DefaultPedOutcomeSim affected_outcome_sim(&affected_control_ped, env);
    agents::GreedyPlanPed affected_plan_ped(&map, &affected_outcome_sim,
                                                     &affected_outcome_calc,
                                                     outcome_sim_params, &reward_calc,
                                                     reward_calc_params, 5, 2.5);
    affected_control_ped.set_ped(affected_ped);
    affected_plan_ped.set_control_ped(&affected_control_ped);

    affected_plan_ped.set_task(task);
    ...
    causal::IEndogenousVariable<FP_DATA_TYPE> *magenta_reward_variable =
            affected_plan_ped.get_real_reward_variable();
    causal::IEndogenousVariable<agents::PedRewards> *magenta_reward_metrics_variable =
            affected_plan_ped.get_real_reward_metrics_variable();
