...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "Usage: ./highd_intervention recording_meta_file_path tracks_meta_file_path "
                     "tracks_file_path start_frame end_frame agent_id intervention_action "
                     "[speed_goal_val speed_goal_time lane_goal_val lane_goal_time]" <<
                     std::endl;
        return -1;
    }
    ...
    if (argc > 8)
    {
        default_fwd_car_action.speed_goal.val = atof(argv[8]);
        default_fwd_car_action.speed_goal.time = temporal::Time(temporal::Duration(atoi(argv[9])));
        default_fwd_car_action.lane_goal.val = atoi(argv[10]);
    ...
    for (i = 0; i < fwd_cars->count(); ++i)
    {
        ...
        if (res && agent_id == id)
        {
            ...
            size_t intervention_action = std::atoi(argv[7]);
            ...
            for (j = 0; j < fwd_car_actions->count(); ++j)
            {
                ...
                else if (j < intervention_action && argc == 8)
                {
