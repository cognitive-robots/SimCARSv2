...
int main(int argc, char *argv[])
{
    if (argc < 7)
    {
        std::cerr << "Usage: ./highd_action_explanation intervention_impact_threshold "
                     "recording_meta_file_path tracks_meta_file_path "
    ...
    agents::highd::HighDFWDCarScene scene(argv[3], argv[4], start_frame, end_frame);
    ...
    if (argc > 7)
    {
        selected_agent_id = std::atoll(argv[7]);
    }
    ...
    if (argc > 8)
    {
        std::string output_json_meta_file_path_str = argv[8];
