...
int main(int argc, char *argv[])
{
    if (argc < 8)
    {
        std::cerr << "Usage: ./laneletd_action_explanation intervention_impact_threshold "
                     "lanelet_map_file_path recording_meta_file_path tracks_meta_file_path "
    ...
    size_t start_frame = atoi(argv[6]);
    size_t end_frame = atoi(argv[7]);
    ...
    agents::otherd::OtherDFWDCarScene scene(argv[3], argv[4], argv[5], start_frame, end_frame);
    ...
    uint64_t selected_agent_id;
    if (argc > 8)
    {
        selected_agent_id = std::atoll(argv[8]);
    }
    ...
    for (u = 0; u < ids->count(); ++u)
    {
        ...
        if (argc > 7 && affected_agent_id != selected_agent_id)
        {
            continue;
        }
    ...
    if (argc > 9)
    {
        std::string output_json_meta_file_path_str = argv[9];
        std::filesystem::path output_json_meta_file_path = std::filesystem::path(output_json_meta_file_path_str);
