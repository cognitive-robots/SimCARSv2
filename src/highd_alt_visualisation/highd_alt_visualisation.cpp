...
int main(int argc, char *argv[])
{
    ...
    uint64_t causing_agent_id = std::atoll(argv[6]);
    uint64_t causing_agent_action = std::atoll(argv[7]);
    uint64_t affected_agent_id = std::atoll(argv[8]);
    uint64_t affected_agent_action = std::atoll(argv[9]);
    ...
    agents::TimeFWDCarActionPair causing_time_action_pair = (*causing_time_action_pairs)[causing_agent_action];
    ...
    for (size_t i = 0; i < causing_agent_action; ++i)
    {
        agents::TimeFWDCarActionPair current_action = (*causing_time_action_pairs)[i];
    ...
    for (size_t i = 0; i < fwd_cars->count(); ++i)
    {
        ...
        {
            map_scene_widget->insert(&causing_fwd_car_sim);
        }
