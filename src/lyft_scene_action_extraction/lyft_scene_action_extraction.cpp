...
int main(int argc, char* argv[])
{
    if (argc < 3)
    {
        std::cerr << "Usage: ./lyft_scene_action_extraction input_scene_file_path output_scene_file_path" << std::endl;
        return -1;
    }
    ...
    try
    {
        scene_with_actions = agent::DrivingAgentScene::construct_from(scene);
    }
    catch (const std::exception& e)
    {
    ...
    try
    {
        ...
        csv_scene_with_actions->save(argv[2]);
    }
    catch (const std::exception& e)
    {
