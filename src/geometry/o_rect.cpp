...
{
    return align_and_check_bounds(rect);
}
...
void ORect::calc_bounds_virt() const
{
    ...
    FP_DATA_TYPE aligned_half_width = std::abs(get_half_width() * cos_o) +
            std::abs(get_half_height() * sin_o);
    FP_DATA_TYPE aligned_half_height = std::abs(get_half_height() * cos_o) +
            std::abs(get_half_width() * sin_o);
...
{
    ORect aligned_this(get_origin(), get_width(), get_height(), 0.0f);
    Vec aligned_point = get_origin() + trig_buff->get_rot_mat(-orientation) *
            (point - get_origin());
...
{
    return this->align_and_check_bounds(o_rect);
}
...
bool ORect::check_collision(ORect const &o_rect) const
{
    return check_vicinity(o_rect) && check_bounds(o_rect) &&
            this->check_collision_virt(o_rect) && o_rect.check_collision_virt(*this);
}
...
{
    set_origin(rotation_centre + trig_buff->get_rot_mat(rotation) *
               (get_origin() - rotation_centre));
    rotate(rotation);
}

bool ORect::check_encapsulation(Vec const &point) const
{
    return check_vicinity(point) && check_bounds(point) && align_and_check_bounds(point);
}
